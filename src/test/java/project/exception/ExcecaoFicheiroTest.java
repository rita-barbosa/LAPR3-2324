...
public class ExcecaoFicheiroTest {
    ...
    @Test(expected = ExcecaoFicheiro.class)
    public void validarFertirregaIncorreto() throws ExcecaoFicheiro {
        if (LocalDate.of(2023,12,30).getDayOfMonth() % 2 == 0) {
            String[] fert = {"A", "14", "P", "mix1", "2"};
            ExcecaoFicheiro.validarFertirrega(fert);
        } else {
            String[] fert = {"A", "14", "I", "mix1", "2"};
            ExcecaoFicheiro.validarFertirrega(fert);
        }
    }
}